...
pintos_init (void)
{
  ...
  void showcmds(){
    ...
    printf(">>> time:     show number of seconds since Unix epoch\n");
    printf(">>> thread:   print thread statistics.\n");
    printf(">>> priority: show current thread priority.\n");
    printf(">>> ram:      show total RAM available to Pintos.(in kB)\n");
    printf(">>> shutdown: power off the machine immediately.\n");
    printf(">>> exit:     exit the kernel shell and halt the kernel.\n");
    printf(">>> showcmds: list available shell commands\n");
    printf(">>> That's all for now...\n");
  }

  void check_cmd(char *cmd_buff){
    if (strcmp(cmd_buff, "whoami") == 0){
      printf("\n>>> I am Nirmal Bandara. Nice to meet you...");
    }
    else if (strcmp(cmd_buff, "time") == 0){
      int time_secs = rtc_get_time();
      printf ("\n>>> Number of seconds passed since Unix epoch : %d s", time_secs);
    }
    else if (strcmp(cmd_buff, "thread") == 0){
      printf ("\n");
      thread_print_stats();
    }
    else if (strcmp(cmd_buff, "priority") == 0){
      int priority = thread_get_priority();
      printf ("\n>>> priority: %d", priority);
    }
    else if (strcmp(cmd_buff, "ram") == 0){
      printf ("\n>>> Ram: %'"PRIu32" kB RAM...",init_ram_pages * PGSIZE / 1024);
    }                                
    else if (strcmp(cmd_buff, "shutdown") == 0){
      shutdown_power_off();
    }
    ...
    else if (strcmp(cmd_buff, "showcmds") == 0){
      showcmds();
    }
    //else if (strcmp(cmd_buff, "clear") == 0) {
    //  cls();
    //} 
    else{
